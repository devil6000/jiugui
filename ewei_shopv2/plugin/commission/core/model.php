...
define('TM_COMMISSION_ANGENT_SEND', 'TM_COMMISSION_ANGENT_SEND');//需要上级发货通知 常量
...
if (!(class_exists('CommissionModel'))) {
	class CommissionModel extends PluginModel
	{
		...
		public function getInfo($openid, $options = NULL)
		{
			...
			$member['agent_level']=$this->getLevel($openid);
		...
		public function sendMessage($openid = '', $data = array(), $message_type = '')
		{
			...
			if (($message_type == TM_COMMISSION_ANGENT_SEND) && empty($usernotice['commission_angent_send'])) {
				$message = $tm['commission_angent_send'];
				$message = str_replace('[订单号]', $data['order']['ordersn'], $message);
				$message = str_replace('[收货地址]', $data['order']['address']['address_area'], $message);
				$message = str_replace('[收货人]', $data['order']['address']['realname'], $message);
				$message = str_replace('[联系电话]', $data['order']['address']['mobile'], $message);
				$message = str_replace('[单品详情]', $data['order']['goods'], $message);
				$message = str_replace('[时间]', date('Y-m-d H:i:s', $data['agenttime']), $message);
				
				$msg = array(
					'keyword1' => array('value' => (!(empty($tm['ommission_angent_sendtitle'])) ? $tm['ommission_angent_sendtitle'] : '成为分销商通知'), 'color' => '#73a68d'),
					'keyword2' => array('value' => $message, 'color' => '#73a68d')
					);
				return $this->sendNotice($openid, $tm, 'commission_angent_send_advanced', $data, $member, $msg);
			}
		}
		...
		{
			if($tag!='commission_angent_send_advanced'){
			...
			case 'commission_order_finish_advanced':
				...
				$arr['[昵称]'] = $data['nickname'];
				break;
			case 'commission_angent_send_advanced':
				$arr['[订单号]'] =  $data['order']['ordersn'];
				$arr['[收货地址]'] =  $data['order']['address']['address_area'];
				$arr['[收货人]'] =  $data['order']['address']['realname'];
				$arr['[联系电话]'] =  $data['order']['address']['mobile'];
				$arr['[单品详情]'] =  $data['order']['goods'];
				$arr['[时间]'] = date('Y-m-d H:i:s', $data['agenttime']);
				break;
			}
