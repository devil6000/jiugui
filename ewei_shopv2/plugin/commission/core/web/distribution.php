...
class Distribution_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,u.*,p.nickname as parentname,p.avatar as parentavatar,f.follow as followed, f.unfollowtime from ' . tablename('ewei_shop_distribution_user') . ' as u  ' . ' left join ' . tablename('ewei_shop_member') . '  dm  on dm.openid = u.openid ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid  ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' and deleted=0 ' . $condition . ' ORDER BY dm.agenttime desc';
		if (empty($_GPC['export'])) {
			$sql .= ' limit ' . (($pindex - 1) * $psize) . ',' . $psize;
		}

		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(dm.id) from ' . tablename('ewei_shop_distribution_user') . ' as u  ' . ' left join ' . tablename('ewei_shop_member') . '  dm  on dm.openid = u.openid ' . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' and deleted=0 ' . $condition, $params);
	...
	public function delete()
	{
		...
		$members = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_distribution_user') . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);

		foreach ($members as $member) {	
			pdo_update('ewei_shop_distribution_user', array('deleted' => 1), array('id' => $member['id']));
			plog('commission.distribution.delete', '删除配送员 <br/>配送员信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['realname'] . '/' . $member['mobile']);
		}
	...
	public function edit()
	{
		...
		$member = pdo_fetch('SELECT dm.*,u.* FROM ' . tablename('ewei_shop_distribution_user') . ' u  ' . ' left join ' . tablename('ewei_shop_member') . '  dm  on dm.openid = u.openid WHERE u.id = ' . $id . ' AND u.uniacid=' . $_W['uniacid']);
		...
		if($_W['ispost']){
			...
			if(!empty($member)){
				pdo_update('ewei_shop_distribution_user',$data,array('uniacid'=>$_W['uniacid'],'id'=>$member['id']));
				plog('commission.distribution.update', '修改配送员 <br/>配送员信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}else{
				$data['uniacid']=$_W['uniacid'];
				$data['createtime']=time();
				pdo_insert('ewei_shop_distribution_user',$data);
	...
	public function m_query()
	{
		...
		include $this->template('commission/distribution/query');
	}
	...
	public function query()
	{
		...
		if (!empty($kwd)) {
			$condition .= ' AND ( dm.nickname LIKE :keyword or u.realname LIKE :keyword or u.mobile LIKE :keyword )';
			$params[':keyword'] = '%' . $kwd . '%';
		}
		
		$ds = pdo_fetchall('SELECT dm.avatar,u.openid,u.id,u.realname,u.mobile FROM ' . tablename('ewei_shop_distribution_user') . ' as u  ' . ' left join ' . tablename('ewei_shop_member') . ' as dm  on dm.openid = u.openid  WHERE '. $condition .' order by dm.createtime desc',$params);
//		$ds = pdo_fetchall('SELECT id,avatar,nickname,openid,realname,mobile FROM ' . tablename('ewei_shop_member') . ' WHERE 1 ' . $condition . ' order by createtime desc', $params);
		include $this->template('commission/distribution/query');
	}
	public function check()
	{
		...
		foreach ($members as $member) {
			...
			if ($status == 1) {
				pdo_update('ewei_shop_distribution_user', array('status' => 1, 'audit_time' => $time), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
				plog('commission.distribution.check', '审核配送员 <br/>配送员信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/'  . $member['realname'] . '/' . $member['mobile']);
//				$this->model->sendMessage($member['openid'], array('nickname' => $member['nickname'], 'agenttime' => $time), TM_COMMISSION_BECOME);
			}
			else {
				pdo_update('ewei_shop_distribution_user', array('status' => 0, 'audit_time' => 0), array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
				plog('commission.distribution.check', '取消审核配送员 <br/>配送员信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
