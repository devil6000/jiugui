...
class Order_EweiShopV2Page extends WebPage
{
	public function main()
	{
        ...
        $condition = ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 and is_repertory=1';
        $uniacid = $_W['uniacid'];
        ...
        if (!empty($list)) {
            ...
            foreach ($list as &$value) {
                ...
                $send_member=m('member')->getMember($value['agentid']);
                $value['agent_us']=p('commission')->getInfo($send_member['openid']);
                $value['delivery_us']=pdo_fetch('SELECT dm.*,u.* FROM ' . tablename('ewei_shop_distribution_user') . ' u  ' . ' left join ' . tablename('ewei_shop_member') . '  dm  on dm.openid = u.openid WHERE u.id = ' . $value['delivery_id'] . ' AND u.uniacid=' . $_W['uniacid']);
                $s = $value['status'];
                $pt = $value['paytype'];
                $value['statusvalue'] = $s;
                ...
                if (!empty($level) && empty($agentid)) {
                    ...
                    $value['magent'] = p('commission')->getInfo($value['agentid']);
                    $value['magent']['level'] = p('commission')->getLevel($value['agentid']);
                }
