	...
	public function getpage($merchid)
	{
		...
		if (!(empty($shop['indexrecommands']))) {
			...
			if (!(empty($goodids))) {
				...
				$indexrecommands = pdo_fetchall('select id, title, thumb, marketprice, productprice, minprice, total, merchid from ' . tablename('ewei_shop_goods') . ' where id in( ' . $goodids . ' ) and uniacid=:uniacid and merchid=:merchid and status=1 order by instr(\'' . $goodids . '\',id),merchdisplayorder desc', array(':uniacid' => $uniacid, ':merchid' => $merchid));
			}
