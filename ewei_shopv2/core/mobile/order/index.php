...
class Index_EweiShopV2Page extends MobileLoginPage
{
	...
	public function detail()
	{
		...
		if($order['agent_send']==1){
			$order['agent']=m('member')->getMember($order['agentid']);
			$order['agent']['address']=pdo_get('ewei_shop_member_address',array('uniacid'=>$_W['uniacid'],'openid'=>$order['agent']['openid'],'isdefault'=>1));
			$order['distribution']['mobile']=$order['agent']['mobile']?$order['agent']['mobile']:$order['agent']['address']['mobile'];
			$order['distribution']['realname']=$order['agent']['realname']?$order['agent']['realname']:$order['agent']['address']['realname'];
		}
		if($order['agent_send']==2){
			$delivery_us=pdo_fetch('SELECT dm.*,u.* FROM ' . tablename('ewei_shop_distribution_user') . ' u  ' . ' left join ' . tablename('ewei_shop_member') . '  dm  on dm.openid = u.openid WHERE u.id = ' . $order['delivery_id'] . ' AND u.uniacid=' . $_W['uniacid']);
			$order['distribution']['mobile']=$delivery_us['mobile'];
			$order['distribution']['realname']=$delivery_us['realname'];
		}
		if (empty($order)) {
			header('location: ' . mobileUrl('order'));
			exit();
