	...
	public function main()
	{
		...
		if (!($packageid)) {
			...
			if (!($isverify) && !($isvirtual) && !($ismerch)) {
                $carrier_list = pdo_fetchall('select * from ' . tablename('ewei_shop_merch_store') . ' where  uniacid=:uniacid and status=1 and type in(1,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
			    /*
				if (0 < $merch_id) {
